...
int memoria_ram_init(void) {
    tam_memoria = MEMORIA_CTX.config->tam_memoria;
    
    espacio_memoria = malloc(tam_memoria);
    if (!espacio_memoria) {
        log_error(MEMORIA_CTX.logger, "Fallo malloc RAM");
        return -1;
    }
    memset(espacio_memoria, 0, tam_memoria);
    log_info(MEMORIA_CTX.logger, "RAM reservada: %d bytes", tam_memoria);
    return 0;
}
...
bool leer_memoria_fisica(uint32_t dir_fisica, void* buffer, int tamanio) {
    if (dir_fisica + tamanio > tam_memoria) {
        log_error(MEMORIA_CTX.logger, "Segmentation Fault (Read): %d", dir_fisica);
        return false;
    }
...
bool escribir_memoria_fisica(uint32_t dir_fisica, void* data, int tamanio) {
    if (dir_fisica + tamanio > tam_memoria) {
        log_error(MEMORIA_CTX.logger, "Segmentation Fault (Write): %d", dir_fisica);
        return false;
    }
